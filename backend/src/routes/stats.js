...
// Function to compute stats from data
...
// Function to refresh cached stats
...
// Initial load
refreshStats((err) => {
  if (err) console.error("Initial stats load failed:", err);
});

// Watch the data file for changes and refresh cache
fs.watch(DATA_PATH, (eventType) => {
  if (eventType === "change") {
    refreshStats((err) => {
      if (err) console.error("Failed to refresh stats:", err);
      else
        console.log(
          "Stats cache refreshed at",
...
router.get("/", (req, res, next) => {
  ...
  // Fallback: If cache is missing, recompute on demand
  refreshStats((err) => {
